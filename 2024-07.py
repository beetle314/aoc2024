...
def parse(data):
    ...
    for line in data.splitlines():
        lhs, rhs = line.split(': ')
        eqns.append((int(lhs), tuple(map(int, rhs.split()))))
...
def test_a(data):
    ...
    for e in eqns:
        for ops in itertools.product([operator.add, operator.mul], repeat=len(e[1]) - 1):
            value = e[1][0]

            for op, term in zip(ops, e[1][1:]):
                value = op(value, term)
                if value > e[0]:
                    break
...
def test_b(data):
    ...
    for e in eqns:
        for ops in itertools.product([operator.add, operator.mul, concat], repeat=len(e[1]) - 1):
            value = e[1][0]
