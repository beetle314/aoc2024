...
def test_a(data):
    return sum(sum(map(to_priority, set(rs[:len(rs)//2]) & set(rs[len(rs)//2:]))) for rs in data.splitlines())


def test_b(data):
    return sum(sum(map(to_priority, set(rs[0]) & set(rs[1]) & set(rs[2]))) for rs in itertools.batched(data.splitlines(), 3))
